...
class JiraSessionRefresher:
    ...
    def receive_push(self, state: dict):
        """QCL 端调用：接收 Mini 推来的 state，落盘到本机 /tmp/jira-session.json。"""
        meta = state.get("_meta", {})
        self._atomic_write("/tmp/jira-session.json", state)
    ...
    def _session_path_static(user: str | None) -> str:
        if not user:
            return "/tmp/jira-session.json"
        safe = re.sub(r"[^A-Za-z0-9._-]+", "_", user)
        return f"/tmp/jira-session-{safe}.json"
