...
from req_pool_draft_service import ReqPoolDraftService, register_draft_service
from jira_service import jira_service as jira_svc, JiraService
from search_chroma import get_vector_store
from requirements_pool_service import RequirementsPoolService, register_req_pool_service
...
def _write_user_session_file(username: str, cookies: Dict[str, str]) -> None:
    ...
    输出：/tmp/jira-session-{username}.json
    """
    if not username:
        return
    try:
        safe = re.sub(r"[^A-Za-z0-9._-]+", "_", username)
        state_path = f"/tmp/jira-session-{safe}.json"
        state_cookies = []
...
async def log_requests(request: Request, call_next):
    ...
    try:
        request.state.current_user = auth_service.get_user_by_session(session_token) if session_token else None
    except Exception:
...
req_pool_service = RequirementsPoolService(
    vector_store_instance,
    kb_runtime_service=kb_runtime_service,
    design_fact_service=design_fact_service,
)
register_req_pool_service(req_pool_service)
spec_generator = SpecGenerator(vector_store_instance, llm_service)
req_pool_draft_service = ReqPoolDraftService(
    spec_generator,
...
req_pool_service.draft_service = req_pool_draft_service
register_draft_service(req_pool_draft_service)
...
@app.get("/req_pool.html")
def read_req_pool():
    return frontend_html_response("req_pool.html")
...
@app.get("/requirements.html")
def read_requirements_page():
    return frontend_html_response("requirements.html")
...
@app.get("/req-pool-spec.js")
def read_req_pool_spec_script():
    return FileResponse(
        os.path.join(FRONTEND_DIR, "req-pool-spec.js"),
...
def auto_refresh_pm_session(request: Request):
    ...
    safe = _re.sub(r"[^A-Za-z0-9._-]+", "_", pm_user)
    state_path = f"/tmp/pm-session-{safe}.json"
    if not os.path.exists(state_path):
        return {"status": "error", "message": f"未找到 session 文件: {state_path}"}
...
@app.get("/api/reports")
def list_reports(project_key: str = "MYPROJECT", domain_modules: Optional[str] = Query(None)):
    report_dir = WEEKLY_REPORT_DIR if project_key == "MYPROJECT" else os.path.join(WEEKLY_REPORT_DIR, project_key)
    ...
    for f in os.listdir(report_dir):
        if f.endswith(".json"):
            # 新格式: Weekly_Report_YYYY-MM-DD_YYYY-MM-DD.json
            match = re.search(r'Weekly_Report_(\d{4}-\d{2}-\d{2})_(\d{4}-\d{2}-\d{2})', f)
            if match:
                data_start = match.group(1)
                data_end = match.group(2)
                ...
                try:
                    with open(filepath, 'r', encoding='utf-8') as file:
                        ...
                        reports.append({
                            "filename": f,
                            "date": data_start,  # 保持兼容
                            "data_start_date": meta.get('data_start_date', data_start),
                            "data_end_date": meta.get('data_end_date', data_end),
                            "period": meta.get('period', f"{data_start} 至 {data_end}"),
                            "generated_at": meta.get('generated_at', ''),
                            "total_tickets": meta.get('total_tickets', 0),
                            "count_process": meta.get('count_process', 0),
                            "count_transferred": meta.get('count_transferred', 0),
                            "domain_modules": meta.get('domain_modules', [])
                        })
                except:
                    # 读取失败，使用文件名解析的数据
                    reports.append({
                        ...
                        "period": f"{data_start} 至 {data_end}",
                        "generated_at": '',
                        "total_tickets": 0,
                        "count_process": 0,
                        "count_transferred": 0,
                ...
                # 尝试旧格式: Weekly_Report_YYYYMMDD.json 或 Weekly_Report_工作流-周数据-日期格式.csv
                match = re.search(r'Weekly_Report_(\d{8})', f)
                if match:
                ...
                try:
                    with open(filepath, 'r', encoding='utf-8') as file:
                        data = json.load(file)
                        meta = data.get('meta', {})
                        # 尝试从source_file提取日期范围
                        source_file = meta.get('source_file', '')
                        date_match = re.search(r'(\d{4}-\d{2}-\d{2})-(\d{4}-\d{2}-\d{2})', source_file)
                        if date_match:
                            data_start = date_match.group(1)
                            data_end = date_match.group(2)
                            period = f"{data_start} 至 {data_end}"
                        else:
                            # 尝试从其他日期字段提取
                            date_match = re.search(r'(\d{4})-(\d{2})-(\d{2})', source_file)
                            if date_match:
                                data_start = f"{date_match.group(1)}-{date_match.group(2)}-{date_match.group(3)}"
                                data_end = data_start
                                period = data_start
                    ...
                    reports.append({
                        ...
                        "period": date_str if date_str else '未知',
                        "generated_at": '',
                        "total_tickets": 0,
                        "count_process": 0,
                        "count_transferred": 0,
    ...
    # domain_modules 过滤（精确集合匹配）
    if domain_modules is not None:
        filter_mods = {m.strip() for m in domain_modules.split(",") if m.strip()}
        reports = [r for r in reports if set(r.get('domain_modules', [])) == filter_mods]

    # 按数据起始日期倒序
    reports.sort(key=lambda x: x['data_start_date'], reverse=True)
    return reports

@app.get("/api/reports/{filename}")
def get_report(filename: str):
    report_dir = WEEKLY_REPORT_DIR
    ...
    # 兼容处理：将meta中的统计数据复制到stats字段
    if 'stats' not in data and 'meta' in data:
        meta = data.get('meta', {})
        data['stats'] = {
            'total_issues': meta.get('total_tickets', 0),
            'completed': meta.get('count_process', 0),
            'new_requirements': meta.get('count_transferred', 0),
            'ratio_transferred': meta.get('ratio_transferred', 0)
        }
...
@app.get("/api/weekly_report/csv_sources")
def list_weekly_csv_sources():
    """获取可用于生成周报的CSV文件列表"""
    try:
        ...
        for f in os.listdir(src_dir):
            if f.endswith('.csv') and "周数据" in f:
                # 检查是否已生成报告
                date_match = re.search(r'(\d{4}-\d{2}-\d{2})-(\d{4}-\d{2}-\d{2})', f)
                has_report = False
                report_filename = None

                if date_match:
                    expected_report = f"Weekly_Report_{date_match.group(1)}_{date_match.group(2)}.json"
                    report_path = os.path.join(report_dir, expected_report)
                    has_report = os.path.exists(report_path)
                    report_filename = expected_report if has_report else None
                ...
                csv_files.append({
                    ...
                    "data_period": f"{date_match.group(1)} 至 {date_match.group(2)}" if date_match else '未知',
...
# --- Requirement Planning Endpoints ---
...
def setup_jira_session(request: JiraSessionRequest):
    ...
    state_path = "/tmp/jira-session.json"
    with open(state_path, "w") as f:
        _json.dump(state, f)
                    ...
                    try:
                        ...
                        if ck_r.status_code == 200:
                            ...
                            if local_state.get("cookies"):
                                with open("/tmp/jira-session.json", "w") as _f:
                                    _json.dump(local_state, _f)
                                print("[refresh] 已将 lap Chrome cookies 镜像到本机 /tmp/jira-session.json")
...
def proxy_attachment(attachment_id: str, request: Request):
    ...
    try:
        ...
        session_candidates = []
        if cur_user:
            session_candidates.append(f"/tmp/jira-session-{re.sub(r'[^A-Za-z0-9._-]+', '_', cur_user['username'])}.json")
        session_candidates.append("/tmp/jira-session.json")  # 全局兜底
...
@app.delete("/api/monthly_reports/{filename}")
def delete_monthly_report(filename: str):
    """删除指定月报"""
    report_dir = MONTHLY_REPORT_DIR
    ...
    try:
        ...
        if os.path.exists(md_path):
            os.remove(md_path)
        return {"status": "success", "message": f"Monthly report {filename} deleted"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
class StartReportTaskRequest(BaseModel):
    task_type: str  # "weekly" or "monthly"
...
def list_report_tasks(task_type: Optional[str] = None):
    """列出所有报告生成任务"""
    return task_mgr.list_tasks(task_type)


# ==========================================
# Requirement Pool API Endpoints
# ==========================================
class ReqReviewRequest(BaseModel):
    reviewer: str
    decision: str
    comments: str
...
class RequirementFactPacketRequest(BaseModel):
    surface: str = ""
    trigger_action: str = ""
    input_object: str = ""
    requiredness: str = ""
    ...
    config_level: List[str] = Field(default_factory=list)
    related_process_types: List[str] = Field(default_factory=list)
    related_bill_types: List[str] = Field(default_factory=list)
...
class CompetitorDossierRequest(BaseModel):
    vendor: str
    feature_key: str
    feature_summary: str = ""
    supported: bool = False
    ui_touchpoints: List[str] = Field(default_factory=list)
    config_levels: List[str] = Field(default_factory=list)
...
@app.get("/api/requirements_pool")
def get_requirements_pool(
    status: str = None,
    start_date: str = None,
    end_date: str = None
):
    """获取看板数据，支持状态筛选和日期范围筛选"""
    date_range = None
    ...
    return req_pool_service.get_board_data(status, date_range)


@app.post("/api/requirements_pool")
def create_manual_requirement(request: CreateManualRequirementRequest):
    try:
        return req_pool_service.create_manual_requirement(
            request.req_id,
            request.title,
            request.description,
        )
...
@app.get("/api/requirements_pool/{req_id}/facts")
def get_requirement_fact_packet(req_id: str):
    try:
        return req_pool_service.get_requirement_fact_packet(req_id)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc))


@app.put("/api/requirements_pool/{req_id}/facts")
def save_requirement_fact_packet(req_id: str, request: RequirementFactPacketRequest):
    try:
        return req_pool_service.save_requirement_fact_packet(req_id, request.model_dump())
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc))
    except RuntimeError as exc:
        raise HTTPException(status_code=500, detail=str(exc))
...
@app.get("/api/requirements_pool/{req_id}/knowledge_context")
def get_requirement_knowledge_context(req_id: str):
    try:
        return req_pool_service.get_requirement_knowledge_context(req_id)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc))


@app.post("/api/requirements_pool/competitor_dossiers")
def save_competitor_dossier(request: CompetitorDossierRequest):
    try:
        return design_fact_service.save_competitor_dossier(request.model_dump())
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))

@app.get("/api/requirements_pool/stats")
def get_requirements_pool_stats(
    start_date: str = None,
    end_date: str = None
):
    """获取需求池统计信息（用于前端显示调试信息），支持日期范围筛选"""
    date_range = None
    if start_date or end_date:
        date_range = {
            'start': start_date,
    ...
    return req_pool_service.get_stats(date_range)

@app.get("/api/requirements_pool/all")
def get_all_requirements():
    """获取所有需求（用于管理/调试）"""
    return req_pool_service.get_all_requirements()

@app.get("/api/requirements_pool/pending_notify")
def get_pending_notify_requirements():
    """返回分析完成但未推送飞书的需求（供 Mac Mini OpenClaw 轮询）"""
    try:
        reqs = req_pool_service.get_pending_notify()
        return {
            "count": len(reqs),
            "requirements": [
                {
                    ...
                    "source_issues": r.get("source_issues", []),
...
@app.delete("/api/requirements_pool/{req_id}")
def delete_requirement(req_id: str):
    """删除指定的需求"""
    success = req_pool_service.delete_requirement(req_id)
    if success:
        return {"message": f"Requirement {req_id} deleted successfully"}
    raise HTTPException(status_code=404, detail=f"Requirement {req_id} not found")

@app.delete("/api/requirements_pool")
def clear_all_requirements(request: Request):
    """清空所有需求（谨慎使用，用于重新生成）"""
    require_admin_user(request)
    count = req_pool_service.clear_all_requirements()
    return {"message": f"Cleared {count} requirements", "count": count}

@app.post("/api/requirements_pool/extract")
def extract_requirements(request: Request):
    """从周报提取需求（保留向后兼容）"""
    require_admin_user(request)
    count = req_pool_service.extract_from_weekly()
    if count == 0:
        return {
            "message": "未提取到需求，可能是数据源不存在或没有符合条件的工单",
            "count": 0,
            "status": "no_data"
        }
    return {"message": f"Successfully extracted {count} requirements", "count": count, "status": "success"}

@app.post("/api/requirements_pool/extract_from_src")
def extract_from_src_files(request: DateRangeFilter = None, raw_request: Request = None):
    ...
    count = req_pool_service.extract_from_src_csv(date_range)
    if count == 0:
        return {
            "message": "未提取到需求，请检查src目录下的CSV文件",
            "count": 0,
            "status": "no_data"
        }
    return {"message": f"从src目录提取完成，新增 {count} 条需求", "count": count, "status": "success"}

class BatchStatusRequest(BaseModel):
    req_ids: List[str]
    new_status: str

@app.post("/api/requirements_pool/batch_status")
def batch_update_status(request: BatchStatusRequest):
    """批量更新需求状态"""
    success_count = req_pool_service.batch_update_status(request.req_ids, request.new_status)
    return {
        "message": f"批量更新完成，成功 {success_count}/{len(request.req_ids)} 条",
        "success_count": success_count,
        "total_count": len(request.req_ids)
    }
...
@app.post("/api/requirements_pool/batch_analyze")
async def batch_analyze_requirements(request: BatchAnalyzeRequest):
    """批量分析需求"""
    llm_runtime = resolve_effective_llm_runtime(
        provider=request.provider,
    ...
    try:
        result = await req_pool_service.analyze_batch(request.req_ids, llm_config)
        return {
            "status": "success",
            "message": f"成功分析 {len(request.req_ids)} 个需求",
            "data": result
        }
    except Exception as e:
...
@app.post("/api/requirements_pool/export")
def export_requirements(request: ExportRequirementsRequest = None):
    """导出需求文档"""
    try:
        req_ids = request.req_ids if request else None
        status_filter = request.status_filter if request else None

        output_path = req_pool_service.export_to_markdown(
            req_ids=req_ids,
...
@app.post("/api/requirements_pool/{req_id}/analyze")
def analyze_requirement(req_id: str, request: AnalyzeRequirementRequest = None):
    """触发需求深度分析后台任务，支持自定义LLM配置"""
    ...
    llm_runtime = resolve_effective_llm_runtime(
        ...
        model_name=request_payload.get("modelName", ""),
        base_url=request_payload.get("baseUrl", ""),
    )
    llm_config = {
        "provider": llm_runtime["provider"],
        "apiKey": llm_runtime["api_key"],
    ...
    try:
        task_id = req_pool_service.start_analysis_task(req_id, llm_config)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc))
...
@app.get("/api/requirements_pool/analyze/{task_id}")
def get_requirement_analysis_task(task_id: str):
    task = req_pool_service.get_analysis_task_status(task_id)
    if task is None:
        raise HTTPException(status_code=404, detail="Analysis task not found")
    return task

@app.post("/api/requirements_pool/{req_id}/review")
def review_requirement(req_id: str, request: ReqReviewRequest):
    success = req_pool_service.submit_review(
        req_id, request.reviewer, request.decision, request.comments, request.expected_version
    )
    if success:
        return {"message": "Review submitted successfully"}
    raise HTTPException(status_code=500, detail="Review submission failed")

@app.post("/api/requirements_pool/{req_id}/mark_notified")
def mark_requirement_notified(req_id: str):
    """标记需求已推送飞书（由 Mac Mini OpenClaw 回调）"""
    ok = req_pool_service.mark_feishu_notified(req_id)
    if not ok:
        raise HTTPException(status_code=404, detail=f"需求 {req_id} 不存在")
    return {"marked": True, "req_id": req_id}


class GenerateSpecRequest(BaseModel):
    formats: List[str] = ["md"]  # 支持 ["md", "docx", "pdf"]
...
class DraftReadyRequest(BaseModel):
    operator: str = "产品经理"


class SubmitDraftRequest(BaseModel):
    submitted_by: str = "产品经理"
...
class CompetitorValidationRequest(BaseModel):
    ...
    vendor: str
    target_url: str
    capture_mode: str = "public_capture"
    profile_name: str = ""
    focus_hint: str = ""
    steps: List[Dict[str, Any]] = Field(default_factory=list)

@app.post("/api/requirements_pool/{req_id}/generate_spec")
def generate_requirement_spec(req_id: str, request: GenerateSpecRequest = None):
    """为需求生成规格文档，支持多种格式"""
    if request is None:
        request = GenerateSpecRequest()

    formats = request.formats if request.formats else ["md"]
    ...
    logger.info("generate_requirement_spec enter req_id=%s formats=%s", req_id, formats)
    task_id = spec_generator.start_generation_task(req_id, formats)
    logger.info("generate_requirement_spec return req_id=%s task_id=%s", req_id, task_id)
    return {
        "status": "accepted",
        "message": "Spec generation task started",
        "task_id": task_id
    }


@app.post("/api/requirements_pool/{req_id}/generate_draft")
def generate_requirement_draft(req_id: str, request: GenerateDraftRequest):
    llm_runtime = resolve_effective_llm_runtime(
        provider=request.provider,
        api_key=request.apiKey,
        model_name=request.modelName,
    ...
    try:
        task_id = req_pool_draft_service.start_generation_task(req_id, request.draft_type, llm_config=llm_config)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))

    return {
        "status": "accepted",
        "message": "Draft generation task started",
        "task_id": task_id,
    }


@app.post("/api/requirements_pool/{req_id}/drafts")
def create_requirement_draft(req_id: str, request: GenerateDraftRequest):
    return generate_requirement_draft(req_id, request)


@app.post("/api/requirements_pool/{req_id}/drafts/{draft_id}/revise")
def revise_requirement_draft(req_id: str, draft_id: str, request: ReviseDraftRequest):
    try:
        task_id = req_pool_draft_service.start_revision_task(req_id, draft_id, request.revision_comments)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    return {
        "status": "accepted",
        "message": "Draft revision task started",
        "task_id": task_id,
    }
...
@app.post("/api/requirements_pool/{req_id}/drafts/{draft_id}/select_solution")
def select_draft_solution(req_id: str, draft_id: str, request: SelectSolutionRequest):
    """选择初稿推荐方案（PRD 生成前必须选择）"""
    try:
        result = req_pool_draft_service.select_solution(
            req_id, draft_id, request.solution_index, request.notes
        )
        return {"status": "success", "data": result}
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
...
@app.post("/api/requirements_pool/{req_id}/drafts/{draft_id}/ready")
def mark_requirement_draft_ready(req_id: str, draft_id: str, request: DraftReadyRequest, raw_request: Request):
    try:
        require_admin_user(raw_request)
        return req_pool_draft_service.mark_draft_ready(req_id, draft_id, request.operator)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))


@app.post("/api/requirements_pool/{req_id}/drafts/{draft_id}/submit")
def submit_requirement_draft(req_id: str, draft_id: str, request: SubmitDraftRequest, raw_request: Request):
    try:
        require_admin_user(raw_request)
        return req_pool_draft_service.submit_draft_to_planning(req_id, draft_id, request.submitted_by)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))


@app.post("/api/requirements_pool/{req_id}/drafts/{draft_id}/confirm_overwrite")
def confirm_requirement_draft_overwrite(req_id: str, draft_id: str, request: Request):
    try:
        require_admin_user(request)
        return req_pool_draft_service.confirm_overwrite(req_id, draft_id)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))


@app.get("/api/requirements_pool/draft/{task_id}")
def get_requirement_draft_task(task_id: str):
    task = req_pool_draft_service.get_task_status(task_id)
    if task is None:
        raise HTTPException(status_code=404, detail="Draft task not found")
    return task


@app.get("/api/requirements_pool/{req_id}/drafts")
def list_requirement_drafts(req_id: str):
    return req_pool_draft_service.list_drafts(req_id)


@app.get("/api/requirements/context")
def get_requirements_context(draft_id: str):
    context = req_pool_draft_service.get_context(draft_id)
    if context is None:
        raise HTTPException(status_code=404, detail="Draft context not found")
    return context


@app.post("/api/requirements_pool/{req_id}/competitor_validation")
def start_competitor_validation(req_id: str, request: CompetitorValidationRequest):
    task_id = competitor_validation_service.start_validation(req_id, request.model_dump())
    return {
        "status": "accepted",
        "task_id": task_id,
    }
...
@app.get("/api/requirements_pool/competitor_validation/{task_id}")
def get_competitor_validation_status(task_id: str):
    task = competitor_validation_service.get_task_status(task_id)
    if task is None:
        raise HTTPException(status_code=404, detail="Competitor validation task not found")
    return task


@app.get("/api/requirements_pool/competitor/tools/health")
def get_competitor_tool_health():
    return competitor_validation_service.get_tool_health()


@app.get("/api/requirements_pool/generate_spec/{task_id}")
def get_generation_task_status(task_id: str):
    task = spec_generator.get_task_status(task_id)
    if task:
        return task
    raise HTTPException(status_code=404, detail="Generation task not found")


@app.post("/api/requirements_pool/generate_spec/{task_id}/retry")
def retry_generation_task(task_id: str):
    result = spec_generator.retry_generation_task(task_id)
    if result:
        return result
    raise HTTPException(status_code=404, detail="Generation task not found")
...
class AggregateAnalyzeRequest(BaseModel):
    scope: str = "all_new"
    req_ids: Optional[List[str]] = None
    theme_hints: Optional[List[str]] = None
    exclude_with_solution: bool = True

@app.post("/api/requirements_pool/aggregate_analyze")
def aggregate_analyze(request: AggregateAnalyzeRequest,
                      exclude_with_solution: bool = Query(True)):
    """启动多智能体聚合分析流水线（cluster→enrich→mc→solution→review）

    exclude_with_solution: 排除已有方案的 cluster 关联的 req_id（query 优先于 body）。
    """
    ...
    try:
        ...
        # query param 优先；FastAPI 默认值不能区分『未传』和『True』，统一以 query 为准
        effective_exclude = exclude_with_solution if exclude_with_solution is not None else request.exclude_with_solution
        parent_task_id, excluded_count = svc.run_pipeline(
            scope=request.scope, req_ids=request.req_ids, theme_hints=request.theme_hints,
            exclude_with_solution=effective_exclude,
        )
        return {"status": "accepted", "parent_task_id": parent_task_id,
...
@app.get("/api/requirements_pool/pipeline/{parent_task_id}")
def get_pipeline_status(parent_task_id: str):
    """查询聚合分析流水线进度"""
    from requirements_pool_aggregate_service import RequirementsPoolAggregateService
    result = RequirementsPoolAggregateService.get_instance().get_pipeline_status(parent_task_id)
    if result.get("error") == "not_found":
        raise HTTPException(status_code=404, detail="pipeline task not found")
    return result

@app.post("/api/requirements_pool/pipeline/{parent_task_id}/themes/confirm")
def confirm_pipeline_themes(parent_task_id: str, body: dict):
    """确认主题列表，解除聚类后的 pending_theme_confirm 阻塞"""
    from requirements_pool_aggregate_service import RequirementsPoolAggregateService
    themes_keep = body.get("themes_keep", [])
    themes_drop = body.get("themes_drop", [])
    ok = RequirementsPoolAggregateService.get_instance().confirm_themes(
        parent_task_id, themes_keep, themes_drop
    )
    if not ok:
        raise HTTPException(status_code=400, detail="pipeline not in waiting_theme_confirm state")
    return {"confirmed": True, "themes_keep": len(themes_keep), "themes_drop": len(themes_drop)}

@app.post("/api/requirements_pool/pipeline/{parent_task_id}/cancel")
def cancel_pipeline(parent_task_id: str, soft: bool = False):
    """
    硬取消流水线：取消任务 + 删除 cluster 产物 + 回滚 req 状态。
    ?soft=1 只 flip SQLite 状态（兼容旧行为）。
    """
    from requirements_pool_aggregate_service import RequirementsPoolAggregateService
    from services.agent_task_store import AgentTaskStore
    svc = RequirementsPoolAggregateService.get_instance()
    if soft:
        ...
        cancelled_parent = store.cancel(parent_task_id)
        children = store.list_children(parent_task_id)
        child_count = sum(1 for c in children if store.cancel(c.id))
        if not cancelled_parent and child_count == 0:
            raise HTTPException(status_code=404, detail="pipeline not found or already in terminal state")
        return {"cancelled": True, "parent": parent_task_id, "children_cancelled": child_count}
    result = svc.cancel_and_cleanup(parent_task_id)
    if not result.get("cancelled"):
        raise HTTPException(status_code=404, detail="pipeline not found or already in terminal state")
    return result

@app.post("/api/requirements_pool/cluster/{cluster_id}/rerun_pipeline")
def rerun_cluster_pipeline(cluster_id: str):
    """重跑单个 cluster 的 Stage 2→3→4，不影响其他 cluster 或整体流水线。"""
    from requirements_pool_aggregate_service import RequirementsPoolAggregateService
    try:
        svc = RequirementsPoolAggregateService.get_instance()
        parent_task_id = svc.kickoff_single_cluster(cluster_id)
        return {"ok": True, "parent_task_id": parent_task_id, "cluster_id": cluster_id}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

class PipelineNoteBody(BaseModel):
    theme_id: str
    stage: str
    note: str

@app.get("/api/requirements_pool/pipeline/{parent_task_id}/notes")
def get_pipeline_notes(parent_task_id: str):
    """读取流水线主题×阶段留言"""
    from requirements_pool_aggregate_service import RequirementsPoolAggregateService
    notes = RequirementsPoolAggregateService.get_instance().get_theme_stage_notes(parent_task_id)
    if notes is None:
        raise HTTPException(status_code=404, detail="pipeline not found")
    return {"notes": notes}

@app.post("/api/requirements_pool/pipeline/{parent_task_id}/notes")
def set_pipeline_note(parent_task_id: str, body: PipelineNoteBody):
    """写入某 (theme, stage) 的留言；阶段已开始则 409"""
    from requirements_pool_aggregate_service import RequirementsPoolAggregateService
    ok, msg = RequirementsPoolAggregateService.get_instance().set_theme_stage_note(
        parent_task_id, body.theme_id, body.stage, body.note
    )
    if not ok:
        raise HTTPException(status_code=409, detail=msg)
    return {"saved": True}

@app.post("/api/requirements_pool/retag_topics")
def retag_topics():
    """对 topic_l1 为空的需求重新调用 TopicParser 打标，写回向量库"""
    import pathlib, json as _json
    from analysis import TopicParser
    vs = req_pool_service.vector_store
    result = vs.req_pool_collection.get(include=["metadatas"])
    if not result or not result.get("metadatas"):
        return {"retagged": 0, "skipped": 0}
    _topic_path = str(pathlib.Path(__file__).parent / "data" / "topic.md")
    try:
        parser = TopicParser(_topic_path)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"TopicParser 初始化失败: {e}")
    retagged = skipped = 0
    ids, metas = result["ids"], result["metadatas"]
    update_ids, update_metas = [], []
    for doc_id, meta in zip(ids, metas):
        if meta.get("topic_l1"):
            skipped += 1
            continue
        title = meta.get("title", "")
        desc  = meta.get("description", "")
        try:
            res = parser.classify_ticket_with_leaf_priority(title, desc or "")
            path = (res or {}).get("full_path") or ""
            ...
            l1, l2 = "未分类", ""
        new_meta = dict(meta)
        new_meta["topic_l1"] = l1
        new_meta["topic_l2"] = l2
        update_ids.append(doc_id)
        update_metas.append(new_meta)
        retagged += 1
    if update_ids:
        vs.req_pool_collection.update(ids=update_ids, metadatas=update_metas)
    return {"retagged": retagged, "skipped": skipped}

@app.get("/api/requirements_pool/themes")
def list_themes():
    """列出所有已产出的需求主题（cluster 列表）"""
    vs = req_pool_service.vector_store
    clusters = vs.list_clusters()
    try:
        from services.topic_codec import resolve_topic_label
        for c in clusters:
            label = resolve_topic_label(c.get("topic_l1", ""), c.get("topic_l2", ""))
            c["topic_l1_name"] = label["l1_name"]
            c["topic_l2_name"] = label["l2_name"]
            c["topic_display"] = label["display"]
    except Exception as e:
        print(f"[list_themes] topic_codec inject failed: {e}")
    return {"themes": clusters, "total": len(clusters)}
...
export_service = get_export_service(llm_service)

class ExportReportRequest(BaseModel):
    report_type: str  # "weekly" | "monthly"
    report_id: str
    formats: List[str] = ["md"]  # ["md", "docx", "pdf", "xlsx"]

@app.post("/api/export/report")
def start_export_report(request: ExportReportRequest, http_request: Request):
    """启动报告导出任务"""
    try:
        # Validate report_type
        if request.report_type not in ["weekly", "monthly"]:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid report_type: {request.report_type}. Must be 'weekly' or 'monthly'"
            )

        # Validate formats
        valid_formats = ["md", "docx", "pdf", "xlsx"]
        invalid_formats = [f for f in request.formats if f not in valid_formats]
        if invalid_formats:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid formats: {invalid_formats}. Valid formats: {valid_formats}"
        ...
        task_id = export_service.start_export(
            request.report_type,
            request.report_id,
            request.formats,
            app_base_url=str(http_request.base_url).rstrip("/"),
        )
...
@app.get("/api/export/download/{task_id}/{filename}")
def download_export_file(task_id: str, filename: str):
    """下载导出文件"""
    ...
    if filename not in task.get("output_files", []):
        raise HTTPException(status_code=403, detail="File not associated with this task")

    # Get file path
    file_path = export_service.get_export_file(task_id, filename)
    if not file_path:
        raise HTTPException(status_code=404, detail="File not found")
...
def list_export_tasks(limit: int = 10):
    """列出最近的导出任务"""
    tasks = []
    for task_id, task in sorted(
        export_service.tasks.items(),
        key=lambda x: x[1].created_at,
        reverse=True
    )[:limit]:
        tasks.append(export_service.get_task_status(task_id))
    return {"tasks": tasks}
...
if _RUN_BG and ENABLE_PM_SCHEDULER:
    try:
...
if _RUN_BG:
    ...
    print("[Automation] 自动化规则轮询已跳过（由 jobmaster daemon 负责）")

# 启动 v4.0 定时调度器
ENABLE_SCHEDULER = os.environ.get("ENABLE_SCHEDULER", "true").lower() == "true"
if _RUN_BG and ENABLE_SCHEDULER:
    try:
        from services.scheduler_service import start_scheduler, register_task_handler

        # 注册任务处理器：需求池分析
        def _task_requirement_pool_analysis(project: str = "MYPROJECT", min_new_count: int = 3, notify_users: list = None, **kwargs):
            """定时触发需求池分析并推送到飞书"""
            from services.feishu_interaction_service import get_interaction_service
            interaction = get_interaction_service()
            interaction.notifier.send_message(
                f"🤖 **定时需求池分析** 已触发\n"
                f"项目: {project} | 最少新需求: {min_new_count}\n"
                f"分析正在后台执行，完成后将推送分析卡片。"
            )

        register_task_handler("requirement_pool_analysis", _task_requirement_pool_analysis)

        # 注册任务处理器：Darwin reqpool 进化评估
        def _task_darwin_reqpool_eval(max_rounds: int = 2, notify_on_complete: bool = True, **kwargs):
            """定时触发 Darwin reqpool 评估，使用 feature routing 配置的 LLM"""
            from services.local_llm_lifecycle import with_fallback, shutdown_if_started_by_us
            from services.feishu_notifier import get_notifier
            notifier = get_notifier()
            provider = with_fallback("darwin_reqpool_eval")
            os.environ["LLM_PROVIDER_OVERRIDE"] = provider
            try:
                notifier.send_message("🧬 Darwin reqpool 评估已触发，后台执行中...")
                from evolution_core.evaluator_agent import evaluate
                ...
                logger.info(f"[Scheduler] Darwin eval done: {scores}")
            except Exception as e:
                logger.error(f"[Scheduler] Darwin eval failed: {e}")
                notifier.send_message(f"❌ Darwin reqpool 评估失败: {e}")
            finally:
                shutdown_if_started_by_us("darwin_reqpool_eval")
        ...
        def _task_reply_training(questions: int = 300, stop_hour: int = 7,
            ...
            _proj_root = str(Path(__file__).parents[1])
            _py = "/Volumes/MacMini/opt/miniconda3/envs/antigravity/bin/python3.12"
            _cmd = [_py, _script, "--questions", str(questions), "--stop-hour", str(stop_hour)]
            _env = {**os.environ, "QCL_BACKEND_URL": "http://ticket.spux.cn",
                    "LLM_PROVIDER_OVERRIDE": provider}
            try:
                result = subprocess.run(
                    _cmd, capture_output=True, text=True,
            ...
            if run_backfill:
                try:
                    ...
                    _bcmd = [_py, _diff_script, "--backfill", "--input", _fb_input,
...
try:
    ...
    for _agent_cls in [DarwinAgent, KbFactAgent, AdoptedAgent, CompetitorAgent]:
        try:
            ...
            print(f"[AgentRegistry] register {_agent_cls.__name__} failed: {_ae}")
    for _agent_cls, _args in [(ReplyAgent, (board_service,)),
                               (HandoverSuggestAgent, (board_service,))]:
        try:
            _reg.register(_agent_cls(*_args))
        except Exception as _ae:
            print(f"[AgentRegistry] register {_agent_cls.__name__} failed: {_ae}")
    try:
        from agents.req_analyst_agent import ReqAnalystAgent
        _reg.register(ReqAnalystAgent(vector_store_instance))
    except Exception as _ae:
        print(f"[AgentRegistry] register ReqAnalystAgent failed: {_ae}")
    for _cls_name, _mod in [
        ("ReqClusterAgent", "agents.req_cluster_agent"),
        ("ReqEnricherAgent", "agents.req_enricher_agent"),
        ("ReqSolutionAgent", "agents.req_solution_agent"),
    ]:
        try:
            import importlib as _il
            _m = _il.import_module(_mod)
            _reg.register(getattr(_m, _cls_name)(vector_store_instance))
        except Exception as _ae:
            print(f"[AgentRegistry] register {_cls_name} failed: {_ae}")

    # ClaudeAgent — Claude Code 原生 bash run_in_background 任务桥接（仅 Mini）
    # 详见 _local/conclusion/misc-temp/AGENT-CLAUDE-NATIVE-V1.md
    try:
        from services.host_context import is_mini as _is_mini
        from agents.claude_agent import ClaudeAgent
        if _is_mini():
            from services.claude_task_scanner import ClaudeTaskScanner
            _claude_scanner = ClaudeTaskScanner()
            _claude_scanner.start()
            _reg.register(ClaudeAgent(_claude_scanner))
            print(f"[AgentRegistry] ClaudeAgent scanner started, task_dir={_claude_scanner.project_dir}")
        else:
    ...
    for _cls_name, _mod in [
        ...
        ("PRDDeverAgent", "agents.prd_dever_agent"),
        ("PRDMasterAgent", "agents.prd_master_agent"),
    ]:
        try:
